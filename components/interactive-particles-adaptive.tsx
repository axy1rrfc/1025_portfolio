...
export function InteractiveParticlesAdaptive() {
  ...
  // Adaptive particle generation based on theme
  const particleData = useMemo(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      if (isDark) {
        ...
        // Much darker colors for light mode (better contrast)
        colors[i * 3] = 0.1 + Math.random() * 0.2     // R (very dark)
        colors[i * 3 + 1] = 0.2 + Math.random() * 0.2 // G (very dark)
        colors[i * 3 + 2] = 0.6 + Math.random() * 0.2 // B (darker blue)
      }
    }
